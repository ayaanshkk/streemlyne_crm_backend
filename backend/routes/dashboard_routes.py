...
from models import ClientMaster, OpportunityDetails, Quotation, InvoiceMaster
...
def get_dashboard_summary():
    ...
    total_clients = (
        ClientMaster.query.filter_by(tenant_id=tid).count()
    )
    ...
    all_opps = OpportunityDetails.query.filter_by(tenant_id=tid).all()
    pipeline_value = sum(float(o.estimated_value) for o in all_opps if o.estimated_value)
    active_jobs = len([
        o for o in all_opps
        if o.stage and o.stage.stage_name not in ("Won", "Lost", "Cancelled")
    ])
    won_jobs = len([o for o in all_opps if o.stage and o.stage.stage_name == "Won"])
    stage_breakdown = {}
    for o in all_opps:
        stage = o.stage or "Unknown"
        stage_breakdown[stage] = stage_breakdown.get(stage, 0) + 1

    # ── Quotations (replaces Proposals) ──────────────────────────────────────
    quotations_this_month = 0
    quotations_last_month = 0
    try:
        quotations_this_month = (
            Quotation.query
            .filter(
                Quotation.client_id.in_(tenant_client_ids),
                func.date(Quotation.created_at) >= month_start,
            ).count()
        )
        quotations_last_month = (
            Quotation.query
            .filter(
                Quotation.client_id.in_(tenant_client_ids),
                func.date(Quotation.created_at) >= last_month_start,
                func.date(Quotation.created_at) < last_month_end,
            ).count()
        )
    except Exception:
        pass
    ...
    return jsonify({
        ...
        "proposals": {
            ...
            "last_month": quotations_last_month,
            "pct_change": _pct_change(quotations_this_month, quotations_last_month),
        },
        "revenue": {
