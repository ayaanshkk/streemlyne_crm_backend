...
─────────────────────────────────────────────────────────────────────────────
[PS-001] ROLE SOURCE FIX — get_user_permissions() previously read role IDs
         from Employee_Master.role_ids via user.employee.get_roles(). Now
         delegates to PermissionRepository.get_user_role_ids() which reads
         from User_Role_Mapping — consistent with user_has_permission() and
         require_tenant_owner. See [PR-001] in permission_repository.py for
         full context.
─────────────────────────────────────────────────────────────────────────────
"""
...
class PermissionService:
    ...
    def user_has_permission(self, user: UserMaster, permission_code: str) -> bool:
        ...
        Delegates to PermissionRepository.user_has_permission() which reads
        from User_Role_Mapping (the canonical source). See [PR-001].

        Args:
            user: UserMaster instance
    ...
    def get_user_permissions(self, user: UserMaster) -> List[str]:
        ...
        [PS-001] Previously read from Employee_Master.role_ids via
        user.employee.get_roles(). Now reads from User_Role_Mapping via
        PermissionRepository.get_user_role_ids() for consistency with
        user_has_permission() and require_tenant_owner.

        Args:
            user: UserMaster instance

        Returns:
            Deduplicated list of permission codes held by the user.
        """
        if not user:
            return []

        # [PS-001] Use User_Role_Mapping — the canonical role store.
        role_ids = self.repo.get_user_role_ids(user.user_id)
        if not role_ids:
            return []
        ...
        return list(set(all_permissions))  # deduplicate